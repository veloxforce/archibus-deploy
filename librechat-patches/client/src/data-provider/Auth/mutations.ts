...
/** Bruce token door: same cache reset + success path as useLoginUserMutation. */
export const useBruceLoginMutation = (
  options?: t.MutationOptions<t.TLoginResponse, string, unknown, unknown>,
): UseMutationResult<t.TLoginResponse, unknown, string, unknown> => {
  const queryClient = useQueryClient();
  const clearStates = useClearStates();
  const resetDefaultPreset = useResetRecoilState(store.defaultPreset);
  const setQueriesEnabled = useSetRecoilState<boolean>(store.queriesEnabled);
  return useMutation([MutationKeys.loginUser, 'bruce'], {
    mutationFn: (userToken: string) => dataService.bruceLogin(userToken),
    ...(options || {}),
    onMutate: (vars) => {
      resetDefaultPreset();
      clearStates();
